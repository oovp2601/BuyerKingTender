...
public class TenderManager {
    ...
    private void simulateSellerBids(int tenderId) {
        try {
            // Get tender info
            String tenderSql = "SELECT * FROM tenders WHERE tender_id = ?";
            Connection conn = DatabaseManager.getConnection();
            PreparedStatement pstmt = conn.prepareStatement(tenderSql);
            ...
            if (rs.next()) {
                ...
                // Get random sellers
                String sellerSql = "SELECT seller_id, seller_name FROM sellers ORDER BY RAND() LIMIT 3";
    ...
    public void showTenderStats() {
        try {
            String sql = "SELECT status, COUNT(*) as count FROM tenders GROUP BY status";
